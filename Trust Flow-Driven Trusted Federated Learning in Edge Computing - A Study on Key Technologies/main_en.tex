...
\begin{algorithm}[htbp]
    ...
    \label{alg:phase12}
    \begin{algorithmic}[1]
        \REQUIRE Client set $\mathcal{C}$, current-round parameter updates $\{\Delta W_k\}_{k\in\mathcal{C}}$, previous-round state $RiskEMA^{(t-1)}$
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{4cm} p{8.5cm} p{2cm}}
        ...
        \midrule
        \multirow{2}{*}{\textbf{Base Environment}}       & Total nodes in heterogeneous topology ($K$)                                  & $20$            \\
                                                           & Local SGD parameters ($lr$, $Momentum$)                      & $0.001$, $0.9$       \\
        \midrule
        \multirow{3}{*}{\textbf{Dual-Stream: Phase 1--3}} & TMAA penalty steepness $\lambda$, dead zone $\tau$, decay $\rho$       & $5.0$, $0.1$, $2.0$  \\
                                                           & 2D audit mapping $\alpha_1, \alpha_2$ and $\beta_{fusion}$ & $0.6$, $0.4$, $2.0$  \\
                                                           & History smoothing $\beta_h$ and risk half-life $\beta_r$             & $0.9$, $0.85$        \\
        \midrule
        \multirow{2}{*}{\textbf{Layer Clipping: Phase 4}}         & Score fusion exponents ($\alpha, \beta, \gamma, p$)             & $3.0, 1.0, 0.5, 1.0$ \\
...
\subsection{A.1 Gradient Direction and Amplitude Probe ($r_{grad}$)}
...
\subsection{A.2 Small-Sample Cross-Entropy Probe ($r_{probe}$)}
...
\subsection{A.3 Deep Neuron Backdoor Activation Probe ($r_{trigger}$)}
