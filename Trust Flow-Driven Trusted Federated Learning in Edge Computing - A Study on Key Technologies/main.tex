...
\textbf{(1) 历史效用流（$HistPerf$）}用于长期能力评估，不直接触发硬性封禁。本文引入贝叶斯推断（Beta 信誉系统），将客户端 $k$ 提供高质量更新的概率建模为 Beta 分布 $\text{Beta}(\alpha_k, \beta_k)$。其中 $\alpha_k$ 记录累积的良性贡献，$\beta_k$ 记录异常或低质量行为。每轮基于 $ContentScore_k$ 提取正向增益 $r_{k}^{good}$ 与负向衰减 $r_{k}^{bad}$，并执行贝叶斯后验更新：
\begin{align}
...
\end{align}
其中，$\lambda_h \in (0,1]$ 为历史衰减因子，用于逐步遗忘陈旧行为以适应动态环境。客户端的长期历史效用值最终取为该 Beta 分布的数学期望：
\begin{equation}
    HistPerf_{k}^{(t)} = \frac{\alpha_{k}^{(t)}}{\alpha_{k}^{(t)} + \beta_{k}^{(t)} + \varepsilon}
\end{equation}
当 $HistPerf$ 较低时，说明客户端近期未能提供足够的正向贡献，节点进入临时软隔离，暂停本轮聚合但不被永久移除，以保留潜在有效的长尾信息。
