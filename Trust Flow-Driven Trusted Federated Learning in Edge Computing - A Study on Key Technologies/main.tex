...
\begin{document}
    ...
    \begin{abstract}
        近年来，联邦学习凭借“数据不出本地、仅交换模型参数”的训练范式，为隐私约束下的多方协同建模提供了有效路径，已成为边缘智能的重要支撑。然而，在开放的边缘计算环境中，非独立同分布（Non-IID）数据、终端设备异构性与不可信客户端等因素叠加，使现有鲁棒聚合方法在安全性、鲁棒性与泛化能力之间面临显著权衡。为此，本文从软硬件协同角度提出一种基于全流程信任流的可信联邦聚合机制。首先，融合基于可信执行环境（TEE）的远程证明与运行时监测，在本地模型训练阶段实现客户端强制准入与执行完整性验证；其次，针对潜伏攻击的跨轮识别问题，设计双流正交信誉演化方法，将客户端信任状态分解为表征长期贡献的历史效用流和表征短期异常的瞬时风险流，实现长期稳定性与快速风险熔断的协同；最后，提出基于纯净参考方向的分层差异化聚合策略，通过按层门控、权重重归一化和动态裁剪，在抑制恶意更新传播的同时保留合法异构贡献，缓解 Non-IID 条件下模型泛化能力下降问题。基于 Flower 平台在 CIFAR-10 数据集上的 20 客户端、6 类混合攻击实验结果表明，本文机制在保证模型精度达到 $92.31\% (\pm 0.09\%)$ 的同时，将后门攻击成功率（ASR）控制在 $10.21\% (\pm 0.05\%)$，且正常客户端的永久误杀率趋近于 0。
    \end{abstract}
    ...
    aggregation methods. To address this issue, we propose a trusted federated
    aggregation mechanism based on end-to-end trust flow from a hardware-software
    co-design perspective. First, remote attestation and runtime monitoring based
    on trusted execution environments (TEEs) are integrated to enforce client
    ...
    dual-stream reputation evolution method that decomposes client trust state
    into a historical utility stream for long-term contribution and an instant
    risk stream for short-term anomalous behavior, enabling coordinated long-term
    stability and rapid risk cut-off. Third, we propose a layer-wise
