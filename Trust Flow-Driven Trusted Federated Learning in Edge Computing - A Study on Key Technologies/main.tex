...
客户端准入不仅是一次身份校验，还应反映本地训练过程是否持续可信。本框架将初始可信度拆分为静态准入与动态评估两个维度：前者判断客户端是否具备合法执行环境，后者把训练过程中的行为波动转化为连续信任观测，并通过滤波更新形成可传递的 $TrustScore$。
...
\begin{enumerate}
    ...
    \end{equation}
    高熵值意味着客户端行为序列存在更强的不确定性，可能对应投毒控制、异常梯度扰动或指令分布偏移。
    \item \textbf{动态信任演化（Kalman-based Evolution）}：利用卡尔曼滤波（Kalman Filter）融合前序信任状态与本轮观测。建立信息熵与测量噪声协方差 $R_k$ 的映射关系 $R_k = f(H(\mathcal{X}_k))$。在“预测-更新”循环中：
    \begin{itemize}
...
\end{align}
这里 $Z_k^{(t)}$ 是基于实时行为的观测分，$R_k^{(t)}$ 会随着熵值增加而增大，从而通过增益 $K_k^{(t)}$ 削弱异常观测对信任状态的影响。该设计使阶段一的输出不再只是“通过/拒绝”的二值结果：远程证明失败时，客户端直接被拒绝；远程证明通过但运行序列波动异常时，其 $TrustScore$ 会被压低，并在后续内容审查与聚合门控中受到持续约束。最终，将卡尔曼滤波产生的估计值融合协方差得到用于后续的初始门禁信任分：$TrustScore_k = \hat{T}_k^{(t)} \cdot (1 / (1 + P_k^{(t)}))$。
