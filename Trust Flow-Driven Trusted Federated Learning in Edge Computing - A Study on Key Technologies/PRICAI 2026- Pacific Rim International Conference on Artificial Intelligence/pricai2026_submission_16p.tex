...
Prior work addresses different parts of this problem. Geometric and statistical defenses such as Krum and Trimmed Mean remove outlying updates \cite{blanchard2017krum,yin2018byzantine}, while FLTrust, FoolsGold, RaSA, and related detection schemes use clean references, similarity histories, or adaptive trust \cite{cao2021fltrust,fung2020foolsgold,wang2025rasa,dou2025toward,wang2024federated}. Backdoor-specific methods further examine trigger behavior or collusion patterns \cite{bagdasaryan2020backdoor,wang2019neuralcleanse,flpurifier,roseagg,shieldfl}. These mechanisms remain exposed to two recurring tensions: static software checks cannot guarantee that local execution was genuine, and aggressive filtering often mistakes Non-IID long-tail updates for attacks. TEE-based FL and trusted inference systems provide a hardware root of trust \cite{liao2024verifiable,zhang2025rppfl,r1_tee_integrity,r5_tee_mitigating,r12_iot_tee,xu2021distributed,yan2024efficient,lu2025tmt}, but they usually do not by themselves solve semantic poisoning after a client has been admitted.
...
\begin{tightenumerate}
    ...
    \item A dual-stream reputation model that decouples historical utility from instant attack risk, reducing the tension between malicious recall and permanent false bans.
...
\textbf{Federated learning in edge computing.} In the standard FL workflow, clients complete local training and submit parameter updates to the server, which aggregates them and redistributes the global model. This workflow keeps data local but makes the server heavily dependent on the authenticity and quality of uploaded updates. Edge computing further complicates this setting because clients differ in compute capability, communication stability, and data distribution. Clustered FL, adaptive pruning-expansion, and split federated learning have been proposed to address heterogeneity and resource constraints \cite{r9_clustered,fedpe,parallelsfl}. As the system becomes more open and layered, however, a larger attack surface appears around client admission, local execution, and update aggregation.
...
\textbf{Active defenses and their limits.} Robust aggregation methods typically rely on geometric distances, coordinate-wise statistics, clean anchors, or temporal similarity. Krum and Trimmed Mean reject or suppress outlying updates \cite{blanchard2017krum,yin2018byzantine}; FLTrust uses a server-side clean reference direction \cite{cao2021fltrust}; FoolsGold exploits historical similarity to reduce Sybil impact \cite{fung2020foolsgold}; RaSA and recent detection frameworks further adapt aggregation or secure aggregation under edge-assisted settings \cite{wang2025rasa,dou2025toward,wang2024federated}. Backdoor-oriented defenses, including FLPurifier, RoseAgg, and ShieldFL, improve protection against trigger or collusion patterns \cite{flpurifier,roseagg,shieldfl}. These defenses remain sensitive to strong Non-IID distributions: a strict threshold raises benign false positives, whereas a loose threshold lets stealthy attacks pass.

\textbf{TEE-assisted trusted training.} Trusted execution environments (TEEs), including Intel SGX and ARM TrustZone, provide isolated execution and protected key material. Prior studies show their value in verifiable inference, robust privacy-preserving FL, training integrity, attack mitigation, and industrial IoT trust sharing \cite{liao2024verifiable,zhang2025rppfl,r1_tee_integrity,r5_tee_mitigating,r12_iot_tee,xu2021distributed,yan2024efficient,lu2025tmt}. Nevertheless, TEE admission alone cannot decide whether a genuine training process produced semantically safe model updates. Trust Flow therefore combines hardware-rooted admission with cross-round risk modeling and layer-aware robust aggregation.
...
\textbf{Design goals.} Trust Flow aims to: (i) maintain convergence under mixed poisoning and backdoors; (ii) keep permanent-ban false positive rate (FPR) low for legitimate long-tail clients; and (iii) avoid heavyweight cryptographic overhead while preserving edge deployability. Figures~\ref{fig:system_arch} and \ref{fig:threat_model} summarize the physical workflow and the threat surface.
...
$\mathcal{X}_k=\{\Delta\text{grad},\Delta\text{loss},\text{instr\_dist}\}$ and maps their entropy to measurement uncertainty:
\begin{equation}
    H(\mathcal{X}_k)=-\sum_i p(x_i)\log p(x_i),\qquad
    R_k^{(t)}=\exp(\lambda H(\mathcal{X}_k^{(t)})).
\end{equation}
Following the information-theoretic filtering view \cite{chen2017maximum} and multidimensional trust evaluation \cite{li2026multidimensional}, the trust estimate is updated by
\begin{equation}
...
The server then constructs a clean guiding direction. If a small clean validation gradient is available, $g_{root}=g_{root\_clean}$; otherwise, it uses high-trust and low-risk clients:
\begin{equation}
...
\end{align}
This channel is intentionally slow: a legitimate long-tail node can recover after temporary low content scores. A low $HistPerf$ means the node has recently contributed little; it leads to soft isolation rather than permanent removal. The risk stream is fast and uses a max-probe rule:
\begin{align}
...
\end{equation}
The trigger probe compares high-level feature activations by KL divergence,
$r_{trigger,k}^{(t)}=\mathrm{KL}(\mathcal{H}(A_{base})\|\mathcal{H}(A_k))$,
which helps identify clean-label or semantic backdoors whose low-level gradient directions remain close to benign updates.
...
\begin{tablenotes}
\scriptsize
\item FPR denotes irreversible false bans of benign clients. Temporary SUSPECT/QUARANTINE states are recoverable and reported separately through trajectories.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
Table~\ref{tab:ablation_overhead} shows that a single-stream reputation model suffers from an FPR/TPR tradeoff: aggressive thresholds increase false bans, while conservative thresholds miss sleeper attacks. HistPerf-only protects benign nodes but lacks short-term attack sensitivity. The dual-stream design reaches high recall while keeping permanent false bans low. The overhead is moderate: TrustReports add about 15 KB per upload, and server aggregation time increases from 2.10 s to 4.85 s per round.
