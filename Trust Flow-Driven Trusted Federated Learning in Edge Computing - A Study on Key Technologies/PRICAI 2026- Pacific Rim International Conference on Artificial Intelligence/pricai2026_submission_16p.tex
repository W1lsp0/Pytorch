...
Prior work addresses different parts of this problem. Geometric and statistical defenses such as Krum and Trimmed Mean remove outlying updates \cite{blanchard2017krum,yin2018byzantine}, while FLTrust and RaSA use clean references or adaptive secure aggregation \cite{cao2021fltrust,wang2025rasa}. Backdoor-specific methods further examine trigger behavior or collusion patterns \cite{bagdasaryan2020backdoor,wang2019neuralcleanse,flpurifier,roseagg}. These mechanisms remain exposed to two recurring tensions: static software checks cannot guarantee that local execution was genuine, and aggressive filtering often mistakes Non-IID long-tail updates for attacks. TEE-based FL systems provide a hardware root of trust \cite{zhang2025rppfl,r1_tee_integrity,r5_tee_mitigating,lu2025tmt}, but they usually do not by themselves solve semantic poisoning after a client has been admitted.
...
\textbf{Active defenses and their limits.} Robust aggregation methods typically rely on geometric distances, coordinate-wise statistics, clean anchors, or adaptive aggregation. Krum and Trimmed Mean reject or suppress outlying updates \cite{blanchard2017krum,yin2018byzantine}; FLTrust uses a server-side clean reference direction \cite{cao2021fltrust}; RaSA adapts secure aggregation under edge-assisted settings \cite{wang2025rasa}. Backdoor-oriented defenses, including FLPurifier and RoseAgg, improve protection against trigger or collusion patterns \cite{flpurifier,roseagg}. These defenses remain sensitive to strong Non-IID distributions: a strict threshold raises benign false positives, whereas a loose threshold lets stealthy attacks pass.

\textbf{TEE-assisted trusted training.} Trusted execution environments (TEEs), including Intel SGX and ARM TrustZone, provide isolated execution and protected key material. Prior studies show their value in robust privacy-preserving FL, training integrity, attack mitigation, and trustworthy model training \cite{zhang2025rppfl,r1_tee_integrity,r5_tee_mitigating,lu2025tmt}. Nevertheless, TEE admission alone cannot decide whether a genuine training process produced semantically safe model updates. Trust Flow therefore combines hardware-rooted admission with cross-round risk modeling and layer-aware robust aggregation.
