...
Prior work addresses different parts of this problem. Geometric and statistical defenses such as Krum and Trimmed Mean remove outlying updates \cite{blanchard2017krum,yin2018byzantine}, while FLTrust, FoolsGold, RaSA, and FLDetector use clean references, temporal similarity, adaptive aggregation, or malicious-client detection \cite{cao2021fltrust,fung2020foolsgold,wang2025rasa,fldetector2022}. Backdoor-specific methods further examine deep model behavior, clipping/noising, model poisoning, or collusion patterns \cite{deepsight2022,flame2022,crowdguard2024,flpurifier,roseagg,shieldfl}. These mechanisms remain exposed to two recurring tensions: static software checks cannot guarantee that local execution was genuine, and aggressive filtering often mistakes Non-IID long-tail updates for attacks. TEE-based FL systems provide a hardware root of trust \cite{zhang2025rppfl,r1_tee_integrity,r5_tee_mitigating,xu2021distributed,lu2025tmt}, but they usually do not by themselves solve semantic poisoning after a client has been admitted.
...
\textbf{Federated learning in edge computing.} In the standard FL workflow, clients complete local training and submit parameter updates to the server, which aggregates them and redistributes the global model. This workflow keeps data local but makes the server heavily dependent on the authenticity and quality of uploaded updates. Edge computing further complicates this setting because clients differ in compute capability, communication stability, and data distribution. A more open and layered system also increases the attack surface around client admission, local execution, and update aggregation.
