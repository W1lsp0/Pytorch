...
class TrustScoreManager:
    ...
    def __init__(self, alpha: float = 3.0, beta: float = 1.0, gamma: float = 0.5):
        ...
        self.risk_soft_scale_floor = 0.30
        self.risk_soft_min_hits = 2
        self.risk_soft_relief_margin = 0.04
        self.risk_suspect_quarantine_threshold = 0.68
        self.risk_suspect_quarantine_rounds = 4
        self.risk_suspect_relief_margin = 0.06
        ...
        self.c2_combo_scale_floor = 0.38
        ...
        self.c2_seed_probe_floor = 0.78
        self.c2_seed_peer_floor = 0.28
        self.c2_seed_grad_floor = 0.20
        self.c2_seed_scale_floor = 0.35
        self.c2_seed_round_window = 20
    ...
    def _ensure_history_entry(
        ...
        if client_id not in self.history:
            ...
            entry.setdefault("suspect_high_streak", 0)
    ...
    def fetch_risk_detail(self, client_id: str) -> Dict[str, float | int | bool]:
        ...
        if client_id in self.blacklist:
            return {
                ...
                "c2_seeded": True,
                "suspect_high_streak": self.risk_suspect_quarantine_rounds,
            }
        entry = self._ensure_history_entry(client_id)
        return {
            ...
            "suspect_high_streak": int(entry.get("suspect_high_streak", 0)),
        }
    ...
    def update_risk_history(
        ...
        scale_risk_tail = self._compute_tail_risk(
            update_norms, tail="high", margin_mad=0.8, scale_mad=2.0, min_mad=0.05
        )
        scale_risk_map: Dict[str, float] = {}
        if update_norms:
            norm_vals = np.array(
                [self._safe_float(v) for v in update_norms.values()],
                dtype=np.float64,
            )
            if norm_vals.size > 0:
                norm_median = float(np.median(norm_vals))
                norm_mad = float(np.median(np.abs(norm_vals - norm_median)))
                abs_scale = max(0.35, 2.5 * max(norm_mad, 1e-3), 0.18 * max(norm_median, 1e-6))
                for cid, nv in update_norms.items():
                    norm_val = self._safe_float(nv)
                    abs_dev = max(0.0, norm_val - norm_median)
                    abs_risk = self._clip01(abs_dev / abs_scale)
                    scale_risk_map[cid] = max(scale_risk_tail.get(cid, 0.0), abs_risk)
        elif scale_risk_tail:
            scale_risk_map = dict(scale_risk_tail)
        ...
        for cid, report_risk in report_risks.items():
            ...
            scale_norm_val = self._safe_float(
                update_norms.get(cid, 0.0), 0.0
            ) if update_norms is not None else 0.0
            scale_risk = self._safe_float(scale_risk_map.get(cid, 0.0), 0.0)
            ...
            p_msg += f"Norm={scale_norm_val:.4f}(Risk={scale_risk:.2f}) | "
            ...
            cross_channel_peak = max(
                report_risk, grad_risk, temporal_risk, spectral_risk, scale_risk,
                trigger_risk, sign_risk, peer_effective
            )
            ...
            if not entry.get("c2_seeded", False):
                ...
                seed_by_extreme_drift = (
                    ...
                    and (
                        grad_risk >= self.c2_seed_grad_floor
                        or scale_risk >= self.c2_seed_scale_floor
                    )
                )
            ...
            c2_grad_hit = c2_enabled and client_entropy > 0.95 and (
                grad_risk >= self.c2_combo_grad_floor
                or sign_risk >= 0.15
                or scale_risk >= self.c2_combo_scale_floor
            )
            ...
            c2_mild_signal = c2_enabled and client_entropy > 0.95 and (
                ...
                or scale_risk >= max(0.25, self.c2_combo_scale_floor - 0.10)
                or sign_risk >= 0.12
            )
            ...
            soft_hit_count = (
                ...
                + int(scale_risk >= self.risk_soft_scale_floor)
                + int(sign_risk >= 0.12)
            ...
            suspect_gate = (
                risk_new >= self.risk_suspect_quarantine_threshold
                and soft_hit_count >= max(2, self.risk_soft_min_hits)
            )
            if suspect_gate:
                entry["suspect_high_streak"] += 1
            else:
                suspect_decay = 2 if (
                    risk_new < (self.risk_suspect_quarantine_threshold - self.risk_suspect_relief_margin)
                    and soft_hit_count == 0
                ) else 1
                entry["suspect_high_streak"] = max(0, entry["suspect_high_streak"] - suspect_decay)

            entry["risk_isolated"] = (
                entry["risk_soft_streak"] >= self.risk_soft_rounds
                or entry["suspect_high_streak"] >= self.risk_suspect_quarantine_rounds
            ...
            hard_evidence = (
                ...
                or (scale_risk >= 0.60 and (probe_risk >= 0.30 or peer_effective >= 0.30))
            )
            hard_gate = (
